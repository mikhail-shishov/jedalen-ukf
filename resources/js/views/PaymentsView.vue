...
const balanceText = computed(() => `${(auth.user?.account_balance ?? 0).toFixed(1)} €`);

const mapStripeErrorCode = (code: string | undefined, message: string | undefined): string => {
	if (!code) return t('payments.unknownStripeError');
...
const payWithStripe = async () => {
	...
	try {
		...
		if (statusCode === 422) {
			stripeError.value = mapServerErrorCode(serverMessage);
		} else {
			...
			<div class="payments-grid">
				<div class="payments-left">
					...
					<button
						...
						@click="payWithStripe"
					>
						{{ isProcessing ? 'Spracovanie...' : 'Prejsť na platobnú bránu' }}
					</button>
